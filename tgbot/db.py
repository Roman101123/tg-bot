...
class Database:
    ...
    def register_activity(self, chat_id, func_name, datetime_of_action, date_of_action):
        self.cursor.execute("INSERT INTO user_activities (chat_id, func_name, datetime_of_action, date_of_action) VALUES (?, ?, ?, ?)",
    ...
    def get_count_from_user_activities(self, func_names):
        self.cursor.execute(
            f"SELECT id FROM user_activities WHERE func_name IN ({', '.join('?'*len(func_names))}) GROUP BY chat_id", func_names)
        result = self.cursor.fetchall()
        return len(result)

    def get_count_from_user_activities_in_date(self, func_names, date):
        self.cursor.execute(
            f"SELECT id FROM user_activities WHERE func_name IN ({', '.join('?'*len(func_names))}) AND date_of_action = ? GROUP BY chat_id", (*func_names, date))
        result = self.cursor.fetchall()
        return len(result)

    def add_user(self, chat_id, name, username, register_datetime, register_date):
        self.cursor.execute("INSERT INTO users (chat_id, name, username, register_datetime, register_date) VALUES (?, ?, ?, ?, ?)",
    ...
    def get_count_of_users_without_admins(self, admins):
        self.cursor.execute(
            f"SELECT COUNT(id) AS count_of_users FROM users WHERE chat_id NOT IN ({', '.join('?'*len(admins))})", admins)
        result = self.cursor.fetchone()[0]
        return result
    ...
    def get_not_is_sent_users(self, mailing_id):
        self.cursor.execute("SELECT users.chat_id FROM users WHERE users.id NOT IN (SELECT mailings_users.user_id FROM mailings_users WHERE mailings_users.mailing_id = ?) AND users.register_datetime < (SELECT mailings.create_datetime FROM mailings WHERE id = ?)", (mailing_id, mailing_id))
        result = self.cursor.fetchall()
        return result
    ...
    def add_mailing(self, message_id, chat_id, all_count, create_datetime, type):
        self.cursor.execute(f"INSERT INTO mailings (message_id, chat_id, all_count, success_count, create_datetime, type) VALUES (?, ?, ?, 0, ?, ?)",
    ...
    def get_not_send_mailing(self, user_id, datetime):
        self.cursor.execute("SELECT mailings.id, mailings.message_id, mailings.chat_id FROM mailings WHERE mailings.id NOT IN (SELECT mailings_users.mailing_id FROM mailings_users WHERE mailings_users.user_id = ?) AND mailings.type = 1 AND mailings.create_datetime > ?", (user_id, datetime))
        result = self.cursor.fetchone()
        return result

    def add_mailings_users(self, message_id, chat_id):
        self.cursor.execute(
            "INSERT INTO mailings_users (mailing_id, user_id) VALUES ((SELECT id FROM mailings WHERE message_id = ?), (SELECT id FROM users WHERE chat_id = ?))", (message_id, chat_id))
        self.db.commit()

    def add_mailings_users_with_mailing_id(self, mailing_id, user_id):
        self.cursor.execute(
            "INSERT INTO mailings_users (mailing_id, user_id) VALUES (?, (SELECT id FROM users WHERE chat_id = ?))", (mailing_id, user_id))
        self.db.commit()
    ...
    def is_user_had_sent_notion(self, mailing_id, user_id):
        self.cursor.execute(
            "SELECT (SELECT users.id FROM users WHERE users.chat_id = ?) IN (SELECT mailings_users.user_id FROM mailings_users WHERE mailings_users.mailing_id = ?)", (user_id, mailing_id))
        result = self.cursor.fetchone()

        return result[0]

    def is_user_had_sent_notion_without_mailing_id(self, message_id, user_id):
        self.cursor.execute("SELECT (SELECT users.id FROM users WHERE users.chat_id = ?) IN (SELECT mailings_users.user_id FROM mailings_users WHERE mailings_users.mailing_id = (SELECT mailings.id FROM mailings WHERE mailings.message_id = ?))", (user_id, message_id))
        result = self.cursor.fetchone()
    ...
    def add_end_of_mailing(self, text, chat_id):
        self.cursor.execute(
            "INSERT INTO end_of_mailings (text_of_message, user_id) VALUES (?, (SELECT id FROM users WHERE chat_id = ?))", (text, chat_id))
        self.db.commit()
    ...
    def get_ends_of_mailings(self, chat_id):
        self.cursor.execute(
            "SELECT id, text_of_message FROM end_of_mailings WHERE user_id = (SELECT id FROM users WHERE chat_id = ?)", (chat_id, ))
        result = self.cursor.fetchall()
        return result
        
    def add_block(self, user_id=None, username=None, block_datetime=None):
        self.cursor.execute(
            "INSERT INTO block_users (user_id, username, block_datetime) VALUES (?, ?, ?)", (user_id, username, block_datetime))
        self.db.commit()

    def update_block(self, block_id, block_datetime):
        self.cursor.execute("UPDATE block_users SET active = 1, block_datetime = ? WHERE id = ?", (block_datetime, block_id))
        self.db.commit()

    def update_block_by_username(self, username, block_datetime):
        self.cursor.execute("UPDATE block_users SET active = 1, block_datetime = ? WHERE username = ?", (block_datetime, username))
        self.db.commit()
    ...
    def del_block_by_username(self, username, block_datetime):
        self.cursor.execute("UPDATE block_users SET active = 0, block_datetime = ? WHERE username = ?", (block_datetime, username))
        self.db.commit()

    def del_block_by_id(self, block_id, block_datetime):
        self.cursor.execute("UPDATE block_users SET active = 0, block_datetime = ? WHERE id = ?", (block_datetime, block_id))
        self.db.commit()
    ...
    def is_user_in_block(self, username):
        self.cursor.execute(f"SELECT ? IN (SELECT username FROM block_users)", (username, ))
        result = self.cursor.fetchone()
        return result[0]
    ...
    def get_block_chat_ids(self):
        self.cursor.execute("SELECT users.chat_id FROM users JOIN block_users ON users.id = block_users.user_id WHERE active = 1")
        result = self.cursor.fetchall()
        return result
    
    def get_block_users(self):
        self.cursor.execute("SELECT block_users.id, IIF(block_users.username IS NOT NULL, '@' || block_users.username, block_users.username), users.name, users.id, users.chat_id, users.register_datetime FROM users RIGHT JOIN block_users ON users.id = block_users.user_id ORDER BY block_users.block_datetime DESC")
        result = self.cursor.fetchall()
        return result

    def add_favorite_product(self, chat_id, product_id, dt):
        self.cursor.execute("INSERT INTO products_users VALUES (?, (SELECT id FROM users WHERE chat_id = ?), ?)", (product_id, chat_id, dt))
        self.db.commit()

    def add_favorite_recipe(self, chat_id, recipe_id, dt):
        self.cursor.execute("INSERT INTO recipes_users VALUES (?, (SELECT id FROM users WHERE chat_id = ?), ?)", (recipe_id, chat_id, dt))
        self.db.commit()

    def del_favorite_product(self, chat_id, product_id):
        self.cursor.execute("DELETE FROM products_users WHERE product_id = ? AND user_id = (SELECT id FROM users WHERE chat_id = ?)", (product_id, chat_id))
        self.db.commit()

    def del_favorite_recipe(self, chat_id, recipe_id):
        self.cursor.execute("DELETE FROM recipes_users WHERE recipe_id = ? AND user_id = (SELECT id FROM users WHERE chat_id = ?)", (recipe_id, chat_id))
        self.db.commit()

    def is_product_favorite(self, chat_id, product_id):
        self.cursor.execute("SELECT * FROM products_users WHERE product_id = ? AND user_id = (SELECT id FROM users WHERE chat_id = ?)", (product_id, chat_id))
        result = self.cursor.fetchone()
        return bool(result)
    
    def is_recipe_favorite(self, chat_id, recipe_id):
        self.cursor.execute("SELECT * FROM recipes_users WHERE recipe_id = ? AND user_id = (SELECT id FROM users WHERE chat_id = ?)", (recipe_id, chat_id))
        result = self.cursor.fetchone()
        return bool(result)
    
    def get_favorites_products(self, chat_id):
        self.cursor.execute("SELECT products.id, products.name, products.img, products.link, products.proteins, products.fats, products.carbohydrates, products.calories FROM products JOIN products_users ON products_users.product_id = products.id WHERE products_users.user_id = (SELECT users.id FROM users WHERE users.chat_id = ?) ORDER BY products_users.datetime_to_add DESC", (chat_id, ))
        result = self.cursor.fetchall()
        return result

    def get_favorites_recipes(self, chat_id):
        self.cursor.execute("SELECT recipes.id, recipes.name, recipes.img, recipes.link, recipes.proteins, recipes.fats, recipes.carbohydrates, recipes.calories, recipes.telegraph_url FROM recipes JOIN recipes_users ON recipes_users.recipe_id = recipes.id WHERE recipes_users.user_id = (SELECT users.id FROM users WHERE users.chat_id = ?) ORDER BY recipes_users.datetime_to_add DESC", (chat_id, ))
        result = self.cursor.fetchall()
        return result

    def add_to_diary(self, product_id, recipe_id, chat_id, volume, group, date_to_add):
        self.cursor.execute("INSERT INTO diary (product_id, recipe_id, user_id, volume, \"group\", date_to_add) VALUES (?, ?, (SELECT id FROM users WHERE chat_id = ?), ?, ?, ?)", (product_id, recipe_id, chat_id, volume, group, date_to_add))
        self.db.commit()

    def get_diary(self, chat_id, date_to_add):
        self.cursor.execute("SELECT diary.volume, diary.\"group\", IIF(products.name IS NULL, recipes.name, products.name) AS name, IIF(products.proteins IS NULL, recipes.proteins, products.proteins) AS proteins, IIF(products.fats IS NULL, recipes.fats, products.fats) AS fats, IIF(products.carbohydrates IS NULL, recipes.carbohydrates, products.carbohydrates) AS carbohydrates, IIF(products.calories IS NULL, recipes.calories, products.calories) AS calories FROM diary LEFT JOIN products ON diary.product_id = products.id LEFT JOIN recipes ON diary.recipe_id = recipes.id WHERE diary.user_id = (SELECT id FROM users WHERE chat_id = ?) AND date_to_add = ?", (chat_id, date_to_add))
        result = self.cursor.fetchall()
        return result